...
def _patch_app_core(source: str) -> str:
    ...
    # Replace full-document href navigation with native Streamlit button reruns.
    # The visible nav stays exactly the same; hidden native buttons carry the click so
    # page changes happen over the existing Streamlit session instead of loading a new
    # browser document (the source of the white flash on mobile Safari).
...
html,body,#root,.stApp,.stAppViewContainer,[data-testid="stAppViewContainer"],[data-testid="stMain"],[data-testid="stMainBlockContainer"]{background:#071019!important;background-color:#071019!important;color-scheme:dark!important}
