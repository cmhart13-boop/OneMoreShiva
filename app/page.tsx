...
function Hero(){
  ...
  return <section className="live-hero" aria-label="Shiva fantasy football home">
    ...
    <div className="live-hero-actions">
      <button type="button" className="live-bell" aria-label="Notifications" aria-expanded={alerts} onClick={()=>setAlerts(v=>!v)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/></svg></button>
...
function LeagueStrip({open,leagues}:{open:(target:string)=>void;leagues:SavedLeague[]}){
  ...
  if(!leagues.length)return <section className="og-league-strip og-league-empty" aria-label="League management"><button type="button" className="og-league-manage" onClick={()=>open('League')}><AppIcon name="plus"/><span><b>Connect your league</b><small>ESPN or Sleeper</small></span><i>›</i></button></section>
  return <section className="og-league-strip" aria-label="Saved league selector"><div className="og-league-strip-track">{leagues.map((saved,index)=>{const league=saved.league_data;const team=league?.teams?.find(item=>String(item.id)===String(saved.team_id));const label=team?.name||saved.nickname||saved.league_name||'League';const detail=league?`${league.teams.length} Teams · ${String(saved.provider||league.league.provider).toUpperCase()}`:String(saved.provider).toUpperCase();return <button type="button" className={`og-league-pill ${index===selected?'active':''}`} key={saved.id} onClick={()=>{setSelected(index);if(league)activateLeague(league,saved.team_id)}}><span className="og-league-pill-icon"><AppIcon name="trophy"/></span><span><b>{label}</b><small>{detail}</small></span></button>})}<button type="button" className="og-league-manage compact" onClick={()=>open('League')} aria-label="Manage leagues"><AppIcon name="plus"/><span>Manage</span></button></div>{leagues.length>1&&<div className="og-league-strip-dots">{leagues.map((league,index)=><button key={league.id} type="button" aria-label={`Select league ${index+1}`} className={index===selected?'active':''} onClick={()=>{setSelected(index);if(league.league_data)activateLeague(league.league_data,league.team_id)}}/>)}</div>}</section>
}

function AskHome({open,leagues}:{open:(target:string)=>void;leagues:SavedLeague[]}){
  const [scope,setScope]=useState<AskScope>('league');const [question,setQuestion]=useState('Should I start Jeanty or Skattebo in Week 1?');const [answer,setAnswer]=useState('Start Jeanty. He has the better matchup, higher projected volume, and more upside in Week 1. Skattebo is a reasonable flex, but Jeanty gives you the bigger edge.');const [status,setStatus]=useState('');const input=useRef<HTMLInputElement>(null)
...
function Home({open}:{open:(target:string)=>void}){
  ...
  return <div className="og-home"><Hero/><LeagueStrip open={open} leagues={leagues}/><AskHome open={open} leagues={leagues}/><Shortcuts open={open}/><Dashboard open={open} leagues={leagues}/></div>
}
