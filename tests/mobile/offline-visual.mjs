...
const result = await page.evaluate(() => {
  ...
  const wordmark=document.querySelector('.live-hero-wordmark').getBoundingClientRect()
  ...
  const askInput=document.querySelector('.og-home-composer input')
  return {overflow:document.documentElement.scrollWidth-window.innerWidth,hero:hero.height,wordmarkLeft:wordmark.left,notifications:document.querySelectorAll('.live-bell').length,shortcut:shortcut.height,card:card.height,strip:document.querySelectorAll('.og-league-strip').length,connect:Array.from(document.querySelectorAll('button')).filter(button=>/connect (your )?league/i.test(button.textContent||'')).length,askFont:css('.og-home-ask-title h2','fontSize'),shortcutFont:css('.og-shortcut-copy b','fontSize'),cardFont:css('.og-snapshot-card>header>b','fontSize'),navFont:css('.og-bottom button span','fontSize'),askCollision:askTitle.right-askScope.left,scopeOverflow,askValue:askInput.value,askPlaceholder:askInput.getAttribute('placeholder'),initialAnswer:document.querySelectorAll('.og-home-answer').length,initialClear:document.querySelectorAll('.og-home-clear').length}
})
...
assert.ok(result.wordmarkLeft<=10)
...
assert.equal(result.cardFont,'14px')
assert.equal(result.navFont,'11.05px')
assert.ok(result.askCollision<=0)
assert.ok(result.scopeOverflow.every(value=>value<=0))
assert.equal(result.askValue,'')
assert.equal(result.askPlaceholder,'Ask Shiva a question…')
assert.equal(result.initialAnswer,0)
assert.equal(result.initialClear,0)
