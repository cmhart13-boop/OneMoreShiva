...
async function assertApprovedHome(page:any){
  ...
  await page.evaluate(()=>window.scrollTo(0,document.documentElement.scrollHeight));await page.waitForTimeout(50);const cardBox=await dashboard.locator('.og-snapshot-card').first().boundingBox();const newsBox=await page.locator('.og-fantasy-news').boundingBox();const navBox=await page.locator('.og-bottom').boundingBox();expect(cardBox).not.toBeNull();expect(newsBox).not.toBeNull();expect(navBox).not.toBeNull();expect(cardBox!.height).toBeGreaterThanOrEqual(260);expect(newsBox!.y+newsBox!.height).toBeLessThanOrEqual(navBox!.y+1)
