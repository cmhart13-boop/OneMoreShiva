...
.live-pill{display:inline-flex;align-items:center;gap:7px;color:#dbe1e7;font-size:10px;font-weight:900;border:1px solid var(--line);background:#151b21;padding:7px 9px;border-radius:999px}.live-dot{width:7px;height:7px;border-radius:99px;background:var(--green);box-shadow:0 0 0 4px rgba(33,193,107,.11)}
...
.player-card{display:grid;grid-template-columns:36px minmax(0,1fr) 58px 58px 52px;align-items:center;gap:7px;min-height:58px;padding:7px 9px;margin-bottom:5px;border:1px solid var(--line);border-radius:10px;background:var(--panel)}
...
.player-link{display:block;color:#fff!important;text-decoration:none!important;font-weight:920;font-size:13px;line-height:1.12;padding:2px 0}.player-link:hover{color:#dfff00!important;text-decoration:underline!important}.player-meta{color:var(--muted);font-size:10px;margin-top:3px}.cell-label{color:var(--muted);font-size:8px;text-transform:uppercase;font-weight:850;letter-spacing:.5px}.cell-value{color:#fff;font-size:12px;font-weight:900}
...
.stButton>button{border-radius:8px!important;font-weight:850!important}.stButton>button[kind="primary"]{background:var(--red)!important;border-color:var(--red)!important;color:#fff!important}
[data-testid="stMetric"]{background:#10161c;border:1px solid var(--line);padding:8px 10px;border-radius:9px}
@media(max-width:820px){
 .block-container{padding-left:.45rem;padding-right:.45rem;padding-top:.35rem}.topbar{padding:9px 10px}.brand-sub{display:none}.hero{padding:13px 11px 6px}.hero h1{font-size:27px}.kpi-grid{grid-template-columns:repeat(2,minmax(0,1fr));padding:7px 11px 12px}.kpi{padding:8px 9px}.kpi-value{font-size:16px}
 .player-card{grid-template-columns:32px minmax(0,1fr) 48px 48px;gap:5px;padding:6px 7px}.player-card .bye-cell{display:none}.player-link{font-size:12px}.player-meta{font-size:9px}
 .profile-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.profile-name-big{font-size:25px}
 .board{display:flex;flex-direction:column}.pick{min-height:54px}.desktop-only{display:none}
...
def load_rankings() -> tuple[pd.DataFrame, str]:
    try:
        ...
        rename = {"player_name":"name", "position":"pos"}
...
WEEKLY_COLUMNS = {
    "player_id","player_display_name","player_name","name","position","recent_team","team",
    "season","season_type","week","opponent_team","opponent","fantasy_points_ppr","fantasy_points",
    "passing_yards","passing_tds","interceptions","passing_attempts","completions",
    "rushing_yards","rushing_tds","carries","targets","receptions","receiving_yards","receiving_tds",
    "fumbles_lost","passing_two_point_conversions","rushing_two_point_conversions","receiving_two_point_conversions"
}


@st.cache_data(ttl=21600, show_spinner=False)
def load_weekly() -> pd.DataFrame:
    # Read only columns the app actually uses. This keeps the 2014-2025 master file fast enough for Streamlit Cloud.
    df = pd.read_csv(
        WEEKLY_URL,
        compression="gzip",
        low_memory=False,
        usecols=lambda c: c in WEEKLY_COLUMNS,
    )
    numeric = [
        "season","week","fantasy_points_ppr","fantasy_points","passing_yards","passing_tds","interceptions",
        "passing_attempts","completions","rushing_yards","rushing_tds","carries","targets","receptions",
...
def weekly_for_player(weekly: pd.DataFrame, player_name: str) -> pd.DataFrame:
    ...
    if out.empty:
        # Only use last-name fallback when it resolves to exactly one player name.
        last = name_key(str(player_name).split()[-1])
        if len(last) >= 5:
            nk = weekly[ncol].astype(str).map(name_key)
            candidate_mask = nk.str.endswith(last)
            names = weekly.loc[candidate_mask, ncol].dropna().astype(str).unique().tolist()
            if len(names) == 1:
                out = weekly.loc[candidate_mask].copy()
    sort_cols = [c for c in ("season","week") if c in out.columns]
    return out.sort_values(sort_cols) if sort_cols and not out.empty else out


def espn_ppr_points(frame: pd.DataFrame) -> pd.Series:
    if frame.empty:
        return pd.Series(dtype=float)
    # Prefer the verified PPR total already stored in the master dataset.
    if "fantasy_points_ppr" in frame.columns:
        stored = pd.to_numeric(frame["fantasy_points_ppr"], errors="coerce")
        if stored.notna().any():
            return stored.round(2)
    scoring = {
        "passing_yards":.04,"passing_tds":4.0,"interceptions":-2.0,
        "rushing_yards":.1,"rushing_tds":6.0,"receptions":1.0,
        "receiving_yards":.1,"receiving_tds":6.0,"fumbles_lost":-2.0,
        "passing_two_point_conversions":2.0,"rushing_two_point_conversions":2.0,"receiving_two_point_conversions":2.0,
    }
    total = pd.Series(0.0,index=frame.index)
    found = False
    for c,m in scoring.items():
        if c in frame.columns:
            total += pd.to_numeric(frame[c],errors="coerce").fillna(0)*m
            found = True
    if found:
...
def init_state() -> None:
    defaults = {
        "draft_log":[],"queue":[],"user_slot":3,"team_count":DEFAULT_TEAMS,"rounds":DEFAULT_ROUNDS,
...
def record_pick(player_id:str, team_no:int)->None:
    ...
    st.session_state.draft_log.append({
        "pick":pick_no,"round":(pick_no-1)//st.session_state.team_count+1,"team":team_no,
...
def cpu_pick()->None:
    ...
    pick_no=next_pick_number();team_no=pick_team(pick_no,st.session_state.team_count)
    rng=random.Random(26000+pick_no)
    idx=min(len(pool)-1,max(0,int(abs(rng.gauss(1.4,1.7)))))
    record_pick(str(pool.iloc[idx]["id"]),team_no)
...
def draft_user(player_id:str)->None:
    ...
    if next_pick_number()>total:return
    sim_to_user_pick()
...
def reset_draft()->None:
    st.session_state.draft_log=[];st.session_state.queue=[]
...
def app_header()->None:
    ...
    st.markdown(f"""
    <div class="shiva-shell"><div class="topbar">
    ...
    </div><div class="hero"><h1>Win the draft before it starts.</h1><p>2026 rankings + real 2014–2025 full-PPR weekly history + live draft context.</p></div>
    <div class="kpi-grid"><div class="kpi"><div class="kpi-label">Current Pick</div><div class="kpi-value">{min(pick_no,total)} / {total}</div></div><div class="kpi"><div class="kpi-label">Round</div><div class="kpi-value">{round_no}</div></div><div class="kpi"><div class="kpi-label">Your Slot</div><div class="kpi-value">#{st.session_state.user_slot}</div></div><div class="kpi"><div class="kpi-label">Roster</div><div class="kpi-value">{roster_n} / {st.session_state.rounds}</div></div></div></div>
...
def render_player_rows(df:pd.DataFrame,*,allow_draft:bool=False,queue_mode:bool=False,limit:int=50,return_page:str="Home")->None:
    if df.empty:
        st.info("No players match this view.");return
    for _,row in df.head(limit).iterrows():
        pid=str(row["id"])
        info,action=st.columns([8.6,1.4],vertical_alignment="center")
        with info:
            href=profile_href(row,return_page)
            st.markdown(f"""
            <div class="player-card">
              <div class="rank">{fmt_int(row.get('overall_rank'))}</div>
              <div><a class="player-link" href="{href}" target="_self">{row['name']}</a><div class="player-meta">{pos_badge(row['pos'])}&nbsp;&nbsp;{row['team']} · tap name for profile</div></div>
              <div><div class="cell-label">ADP</div><div class="cell-value">{fmt_num(row.get('draft_adp'))}</div></div>
              <div><div class="cell-label">Pos</div><div class="cell-value">{str(row['pos'])}{fmt_int(row.get('position_rank'))}</div></div>
              <div class="bye-cell"><div class="cell-label">Bye</div><div class="cell-value">{fmt_int(row.get('bye'))}</div></div>
            </div>""",unsafe_allow_html=True)
        with action:
            if allow_draft:
                if st.button("DRAFT",key=f"draft_{pid}",type="primary",use_container_width=True):draft_user(pid);st.rerun()
            elif queue_mode:
                ...
                if st.button("✓" if queued else "+ Queue",key=f"queue_{return_page}_{pid}",use_container_width=True,disabled=queued):st.session_state.queue.append(pid);st.rerun()


def render_board()->None:
    team_count=st.session_state.team_count;rounds=st.session_state.rounds;pick_map={x["pick"]:x for x in st.session_state.draft_log}
    html=['<div class="board-wrap"><div class="board">']
    for round_no in range(1,rounds+1):
        for display_team in range(1,team_count+1):
            pick_no=overall_pick_for_team(round_no,display_team,team_count);pick=pick_map.get(pick_no)
            if pick:
    ...
    html.append("</div></div>");st.markdown("".join(html),unsafe_allow_html=True)
...
def render_roster()->None:
    roster=user_roster();st.markdown('<div class="roster-card"><div class="roster-head"><strong>YOUR TEAM</strong><span>Live draft roster</span></div>',unsafe_allow_html=True)
    for slot,p in assign_roster_slots(roster):
        if p:st.markdown(f'<div class="slot"><div class="slot-name">{slot}</div><div><div class="slot-player">{p["name"]}</div><div class="slot-meta">{p["pos"]} · {p["nfl_team"]}</div></div><div class="slot-meta">Pick {p["pick"]}</div></div>',unsafe_allow_html=True)
        else:st.markdown(f'<div class="slot"><div class="slot-name">{slot}</div><div class="slot-player" style="color:#66707c">Empty</div><div></div></div>',unsafe_allow_html=True)
...
def season_summary(frame:pd.DataFrame)->dict[str,Any]:
    ...
    return {"games":int(len(pts)),"total":float(pts.sum()) if len(pts) else np.nan,"ppg":float(pts.mean()) if len(pts) else np.nan,"median":float(pts.median()) if len(pts) else np.nan,"weeks15":int((pts>=15).sum()) if len(pts) else 0,"weeks20":int((pts>=20).sum()) if len(pts) else 0}
...
def weekly_table(frame:pd.DataFrame,pos:str)->pd.DataFrame:
    ...
    data={"WK":pd.to_numeric(f["week"],errors="coerce").astype("Int64") if "week" in f.columns else pd.Series(range(1,len(f)+1)),"PPR":f["PPR"].round(1)}
    opp=next((c for c in ("opponent_team","opponent") if c in f.columns),None)
    if opp:data["OPP"]=f[opp].fillna("—").astype(str)
    p=str(pos).upper().replace("D/ST","DST")
    pairs=[]
    if p=="QB":pairs=[("PASS YDS","passing_yards"),("PASS TD","passing_tds"),("INT","interceptions"),("RUSH YDS","rushing_yards"),("RUSH TD","rushing_tds")]
    elif p in {"WR","TE"}:pairs=[("TGT","targets"),("REC","receptions"),("REC YDS","receiving_yards"),("REC TD","receiving_tds"),("RUSH YDS","rushing_yards")]
    elif p=="RB":pairs=[("CAR","carries"),("RUSH YDS","rushing_yards"),("RUSH TD","rushing_tds"),("REC","receptions"),("REC YDS","receiving_yards"),("REC TD","receiving_tds")]
    for label,c in pairs:
        if c in f.columns:data[label]=pd.to_numeric(f[c],errors="coerce").fillna(0).round(0).astype(int)
    table=pd.DataFrame(data)
    if "WK" in table.columns:table=table.loc[pd.to_numeric(table["WK"],errors="coerce").between(1,18)].sort_values("WK")
...
def render_profile(player_id:str, player_name_hint:str="", return_page:str="Rankings")->None:
    ...
    if match.empty and player_name_hint:
        match=players.loc[players["name"].astype(str).map(name_key).eq(name_key(player_name_hint))]
    if match.empty:
        st.error("That player is not in the current 2026 ranking feed.");return
    p=match.iloc[0]
    back_page=return_page if return_page in {"Home","Mock Draft","Rankings","Ask Shiva"} else "Rankings"
    st.markdown(f'<a href="?page={quote_plus(back_page)}" target="_self" style="color:#fff;text-decoration:none;font-weight:900">← Back to {back_page}</a>',unsafe_allow_html=True)
    with st.spinner("Loading verified weekly player history…"):
        try:
            weekly=load_weekly();pf=weekly_for_player(weekly,str(p["name"]))
        except Exception as exc:
    ...
    st.markdown(f"""
    <div class="profile-hero"><div>{pos_badge(p['pos'])}</div><div class="profile-name-big">{p['name']}</div><div class="profile-meta">{p['team']} · 2026 ADP {fmt_num(p.get('draft_adp'))} · Overall #{fmt_int(p.get('overall_rank'))} · Bye {fmt_int(p.get('bye'))}</div>
    <div class="profile-stats"><div class="profile-stat"><b>{fmt_num(p.get('draft_adp'))}</b><small>2026 Consensus ADP</small></div><div class="profile-stat"><b>{p['pos']}{fmt_int(p.get('position_rank'))}</b><small>Position Rank</small></div><div class="profile-stat"><b>{len(seasons)}</b><small>Seasons On File</small></div><div class="profile-stat"><b>{'Queued' if p['id'] in st.session_state.queue else ('Drafted' if p['id'] in drafted_ids() else 'Available')}</b><small>Draft Status</small></div></div></div>
    """,unsafe_allow_html=True)
    if pf.empty:
        st.info("No NFL regular-season weekly record exists yet for this player in the 2014–2025 database. This is expected for 2026 rookies and players with no prior NFL regular-season games.");return
    season=st.selectbox("Season",seasons,index=0,key=f"season_{player_id}")
    ...
    m1.metric("PPR PPG",fmt_num(summary["ppg"]));m2.metric("PPR Total",fmt_num(summary["total"]));m3.metric("Games",summary["games"]);m4.metric("15+ Weeks",summary["weeks15"]);m5.metric("20+ Weeks",summary["weeks20"])
    st.caption(f"Verified full-PPR weekly dataset · Age entering {season}: {player_birth_age(str(p['name']),int(season))} · Regular season only")
    table=weekly_table(sf,str(p["pos"]))
    if view=="Weekly Stats":
        if not table.empty and {"WK","PPR"}.issubset(table.columns):
            fig=px.line(table,x="WK",y="PPR",markers=True,title=f"{p['name']} — {season} weekly PPR")
            ...
            fig.update_traces(line=dict(width=3));st.plotly_chart(fig,use_container_width=True)
        st.dataframe(table,use_container_width=True,hide_index=True,height=min(620,38+35*len(table)))
    else:
        career=[]
        for yr in seasons:
            yf=pf.loc[pd.to_numeric(pf["season"],errors="coerce").eq(int(yr))];s=season_summary(yf)
            career.append({"Season":yr,"Games":s["games"],"PPR Total":round(s["total"],1),"PPR PPG":round(s["ppg"],1),"15+":s["weeks15"],"20+":s["weeks20"]})
        ...
        if not table.empty:
            ...
            fig.update_layout(template="plotly_dark",height=300,margin=dict(l=8,r=8,t=42,b=8),paper_bgcolor="#080b0f",plot_bgcolor="#080b0f",xaxis=dict(dtick=1));st.plotly_chart(fig,use_container_width=True)


# =============================================================================
# ASK SHIVA — GROUNDED IN APP DATA
# =============================================================================
def mentioned_players(question:str,limit:int=4)->list[str]:
    q=name_key(question);words=re.findall(r"[a-z0-9]+",question.casefold());found=[]
...
def player_history_context(player_name:str,question:str,weekly:pd.DataFrame)->str:
    ...
    for season in seasons:
        sf=pf.loc[pd.to_numeric(pf["season"],errors="coerce").eq(season)];s=season_summary(sf);weekly_pts=espn_ppr_points(sf).round(1).tolist()
...
def ask_shiva(question:str)->str:
    ...
    if names:
        try:
            weekly=load_weekly();history=[player_history_context(n,question,weekly) for n in names]
    ...
    if len(names)==1 and years and any(t in question.casefold() for t in ("points per game","ppg","average")):
        try:
            pf=weekly_for_player(load_weekly(),names[0]);year=years[0];sf=pf.loc[pd.to_numeric(pf["season"],errors="coerce").eq(year)];s=season_summary(sf)
            if s["games"]:return f"{names[0]} averaged **{s['ppg']:.2f} full-PPR points per game in {year}** across {s['games']} regular-season games ({s['total']:.1f} total PPR points)."
    ...
    if not key or OpenAI is None:
        return ("Verified app data:\n\n"+"\n\n".join(history)+"\n\nAdd OPENAI_API_KEY in Streamlit Secrets for full Shiva analysis.") if history else "Add OPENAI_API_KEY in Streamlit Secrets to enable Shiva recommendations. Player rankings and profiles work independently."
    ...
    system=("You are Shiva, an elite fantasy-football analyst. Default to ESPN full 1-point PPR and 4 points per passing TD. Be decisive and concise. Treat supplied rankings, roster, and historical stats as authoritative. Never invent stats, injuries, transactions, ADP, or news. For draft advice account for positional scarcity, roster construction, ADP opportunity cost, and actual availability. "+f"Draft slot: {st.session_state.user_slot}. Roster: {roster_text}. Top available: {remaining}. Historical context: {history}.")
    try:
        client=OpenAI(api_key=key);response=client.responses.create(model="gpt-5-mini",input=[{"role":"system","content":system},{"role":"user","content":question}]);return response.output_text
...
# URL-driven profile route. This makes every visible player name a real link on desktop and mobile.
qp=st.query_params
player_param=str(qp.get("player") or "")
name_param=str(qp.get("name") or "")
return_param=str(qp.get("return") or "Rankings")
if player_param:
    render_profile(player_param,name_param,return_param)
    st.stop()

page_hint=str(qp.get("page") or "")
pages=["Home","Mock Draft","Rankings","Ask Shiva"]
if page_hint in pages and st.session_state.get("main_page")!=page_hint:
    st.session_state.main_page=page_hint
...
if page=="Home":
    ...
    with a:st.markdown("#### Top Available");render_player_rows(available_df(),limit=10,return_page="Home")
    with b:st.markdown("#### Your Roster");render_roster()

elif page=="Mock Draft":
    settings,controls=st.columns([2.3,1])
    with settings:st.markdown('<div class="section-title"><div><h2>Live Mock Draft</h2><span>10-team snake · real 2026 board · persistent state</span></div></div>',unsafe_allow_html=True)
    with controls:
        x1,x2=st.columns(2)
        with x1:
    ...
    question=st.text_area("What do you want to know?",placeholder="How many PPR points per game did Christian McCaffrey average in 2025?",height=110)
...
st.caption("Shiva Fantasy Football · 2026 ranking feed + verified 2014–2025 weekly player history. Player names are tappable profile links on desktop and mobile.")