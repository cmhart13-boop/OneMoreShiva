...
def render_players(df:pd.DataFrame,ret:str,action:str="none",limit:int=80):
    ...
    for _,r in df.head(limit).iterrows():
        pid=str(r["id"])
        if action=="draft":
            player_card(r,ret,draft_action=True)
        else:
            player_card(r,ret)
        if action=="remove":
