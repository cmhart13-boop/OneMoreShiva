...
export default function CoachHub() {
  ...
  const coachTab = COACH_TAB_MAP[view]
  ...
  return <div className="coach-hub">
    ...
    <div className="coach-hub-tabs" aria-label="Shiva Coach tools">
      {HUB_TABS.map((item) => <button type="button" key={item} className={view === item ? 'active' : ''} onClick={() => setView(item)}>{item}</button>)}
    </div>
    ...
    <div className="coach-hub-content">
      ...
      {coachTab && <CoachView showTabs={false} activeTab={coachTab} />}
    </div>
  </div>
}
