...
export default function DraftView() {
  ...
  if (!started) return <>
    <div className="section-kicker">MOCK DRAFT</div>
    <h1>2026 Shiva Draft</h1>
    ...
    <div className="panel start-draft-card">
      <label htmlFor="draft-slot">Draft position</label>
      <div className="slot-grid">{Array.from({ length: 12 }, (_, index) => index + 1).map((value) => <button key={value} className={slot === value ? 'active' : ''} onClick={() => setSlot(value)}>{value}</button>)}</div>
      <button className="primary-button start-button" onClick={startDraft} disabled={!players.length}>{players.length ? 'Start Mock Draft' : 'Loading draft board…'}</button>
    </div>
  </>
  ...
  return <>
    ...
    {!completed && <section className="draft-iq">
      <div className="draft-iq-head"><div><span>SHIVA DRAFT IQ</span><h2>Best choices right now</h2></div><em>LIVE CONTEXT</em></div>
      {recommendations.length ? <div className="rec-list">{recommendations.map((rec) => <article key={rec.player.id} className="rec-card"><div><span>{rec.label}</span><h3>{rec.player.name}</h3><p>{rec.player.pos} · {rec.player.team} · ADP {rec.player.adp?.toFixed(1) ?? '—'}</p><small>{rec.reason}</small></div><button onClick={() => draftPlayer(rec.player)}>DRAFT</button></article>)}</div> : <p className="muted-copy">Shiva IQ unlocks on your pick.</p>}
    </section>}
    ...
    {poolTab === 'Available' && <>
      <div className="pool-controls"><input aria-label="Search players" placeholder="Search player" value={search} onChange={(event) => setSearch(event.target.value)} /><div className="filter-pills compact-pills">{['ALL','RB','WR','QB','TE'].map((filter) => <button key={filter} className={posFilter === filter ? 'active' : ''} onClick={() => setPosFilter(filter)}>{filter}</button>)}</div></div>
      <div className="player-pool">{available.filter((player) => posFilter === 'ALL' || player.pos === posFilter).filter((player) => !search || player.name.toLowerCase().includes(search.toLowerCase())).slice(0, 60).map((player) => <div className="pool-row" key={player.id}><div className="pool-rank">#{player.rank}</div><span className={`pos-chip pos-${player.pos}`}>{player.pos}</span><div className="pool-name"><b>{player.name}</b><small>{player.team || '—'} · ADP {player.adp?.toFixed(1) ?? '—'}</small></div><div className="pool-actions"><button aria-label={`Queue ${player.name}`} className={queue.includes(player.id) ? 'queued' : ''} onClick={() => setQueue((items) => items.includes(player.id) ? items.filter((id) => id !== player.id) : [...items, player.id])}>☆</button><button disabled={!isUserPick} onClick={() => draftPlayer(player)}>Draft</button></div></div>)}</div>
    </>}

    {poolTab === 'My Roster' && <div className="roster-list">{myPicks.length ? myPicks.map((pick) => <div className="pool-row" key={pick.overall}><div className="pool-rank">R{pick.round}</div><span className={`pos-chip pos-${pick.player.pos}`}>{pick.player.pos}</span><div className="pool-name"><b>{pick.player.name}</b><small>{pick.player.team} · Pick {pick.overall}</small></div></div>) : <div className="empty-state">No picks yet.</div>}</div>}

    {poolTab === 'Queue' && <div className="roster-list">{queue.length ? queue.map((id) => available.find((player) => player.id === id)).filter(Boolean).map((player) => <div className="pool-row" key={player!.id}><div className="pool-rank">#{player!.rank}</div><span className={`pos-chip pos-${player!.pos}`}>{player!.pos}</span><div className="pool-name"><b>{player!.name}</b><small>{player!.team} · ADP {player!.adp?.toFixed(1) ?? '—'}</small></div><div className="pool-actions"><button onClick={() => setQueue((items) => items.filter((item) => item !== player!.id))}>×</button><button disabled={!isUserPick} onClick={() => draftPlayer(player!)}>Draft</button></div></div>) : <div className="empty-state">Your queue is empty.</div>}</div>}
  </>
}
