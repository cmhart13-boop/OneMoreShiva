...
function playerPhoto(id: string) {
...
export default function CoachView() {
  ...
  const openRosterPlayer = (row: LeagueState['roster'][number]) => {
    const ranked = players.find((player) => player.name.toLowerCase() === row.player.toLowerCase())
    setSelectedPlayer({
      id: row.playerId,
      name: row.player,
  ...
  const rosterRows = (rows: LeagueState['roster']) => <div className="espn-roster">{rows.map((row) => <button type="button" className="espn-roster-row clickable-player" key={`${row.playerId}-${row.slot}`} onClick={() => openRosterPlayer(row)}><span className="espn-slot">{row.slot}</span><img className="espn-player-photo" src={playerPhoto(row.playerId)} alt="" onError={(event) => { event.currentTarget.style.visibility = 'hidden' }} /><div className="espn-player-copy"><b>{row.player}</b><small>{PRO_TEAM[row.proTeamId || 0] || ''}{row.injuryStatus ? ` · ${row.injuryStatus}` : ''}</small></div><span className="espn-row-rank">{players.find((player) => player.name.toLowerCase() === row.player.toLowerCase())?.rank ? `#${players.find((player) => player.name.toLowerCase() === row.player.toLowerCase())?.rank}` : ''}</span></button>)}</div>
  ...
  return <>
    ...
    {tab === 'Waivers' && <><div className="section-heading"><div><div className="section-kicker">AVAILABLE TALENT</div><h2>Waiver Wire</h2></div>{league && <span className="live-dot">LEAGUE LIVE</span>}</div>{!league ? <div className="empty-state">Connect ESPN to rank your league’s actual free-agent pool.</div> : <div className="player-pool">{waiverRows.map(({ agent, player }, index) => <button type="button" className="pool-row waiver-photo-row clickable-player" key={agent.playerId || agent.player} onClick={() => setSelectedPlayer({ id:agent.playerId, name:agent.player, team:PRO_TEAM[agent.proTeamId || 0] || player?.team || '', injuryStatus:agent.injuryStatus, percentOwned:agent.percentOwned, percentStarted:agent.percentStarted, rank:player?.rank, pos:player?.pos })}><div className="pool-rank">{index + 1}</div><img className="espn-player-photo" src={playerPhoto(agent.playerId)} alt="" onError={(event) => { event.currentTarget.style.visibility = 'hidden' }} /><span className={`pos-chip pos-${player?.pos || 'NA'}`}>{player?.pos || '—'}</span><div className="pool-name"><b>{agent.player}</b><small>{player ? `${player.team} · Shiva #${player.rank}` : 'ESPN free agent'}{agent.injuryStatus ? ` · ${agent.injuryStatus}` : ''}</small></div><div className="owned"><b>{num(agent.percentOwned,0,'%')}</b><span>Owned</span></div></button>)}</div>}</>}
    ...
    {selectedPlayer && <div className="player-modal-backdrop" role="presentation" onClick={() => setSelectedPlayer(null)}><section className="player-modal" role="dialog" aria-modal="true" aria-label={`${selectedPlayer.name} player details`} onClick={(event) => event.stopPropagation()}><button type="button" className="player-modal-close" aria-label="Close player details" onClick={() => setSelectedPlayer(null)}>×</button><div className="player-modal-hero"><img className="player-modal-photo" src={playerPhoto(selectedPlayer.id)} alt={selectedPlayer.name} onError={(event) => { event.currentTarget.style.visibility = 'hidden' }} /><div><div className="player-modal-kicker">ESPN PLAYER</div><h2>{selectedPlayer.name}</h2><div className="player-modal-meta">{[selectedPlayer.pos, selectedPlayer.team, selectedPlayer.slot ? `${selectedPlayer.slot} slot` : ''].filter(Boolean).join(' · ')}</div></div></div><div className="player-modal-stats"><div><b>{selectedPlayer.rank ? `#${selectedPlayer.rank}` : '—'}</b><span>Shiva Rank</span></div><div><b>{num(selectedPlayer.percentOwned,0,'%')}</b><span>Owned</span></div><div><b>{num(selectedPlayer.percentStarted,0,'%')}</b><span>Started</span></div></div><div className="player-modal-body"><p>{selectedPlayer.injuryStatus ? `ESPN status: ${selectedPlayer.injuryStatus}. ` : ''}Player imagery and roster identity are loaded from the connected ESPN player ID so the same player presentation follows the league data already powering Shiva.</p></div></section></div>}
  </>
}