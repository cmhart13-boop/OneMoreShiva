...
export default function EdgeRankingsView({ mode, onBack, inline = false, playerNames = [], limit = 10 }: EdgeRankingsViewProps) {
  ...
  return <div className={inline ? 'edge-rankings-inline' : 'edge-rankings-view'}>
    {!inline && <>{onBack && <button className="back-link" onClick={onBack}>← Shiva Coach</button>}<div className="section-kicker">SHIVA EDGE</div><h1>{title}</h1><p className="lede">{subtitle}</p></>}

    <div className="filter-pills edge-filter-pills">{FILTERS.map((item) => <button key={item} className={`${filter === item ? 'active ' : ''}edge-filter-${item}`} onClick={() => setFilter(item)}>{item === 'ALL' ? 'ALL' : item}</button>)}</div>

    {loading ? <div className="edge-inline-status">Loading rankings…</div> : rows.length ? <div className="rank-list edge-rank-list">
      {rows.map((player, index) => <button type="button" className="rank-row edge-rank-row has-player-photo clickable-player" key={player.id} onClick={() => setSelectedPlayer({ ...player, id: player.espnId || player.id, ppg: player.ppg })}>
        ...
        <span className="edge-pos-text">{player.pos}</span>
        <PlayerAvatar playerId={player.espnId || player.id} name={player.name} />
        <div><b>{player.name}</b><small>{player.team || '—'} · {player.ppg.toFixed(1)} PPG</small></div>
    ...
    </div> : <div className="edge-inline-status">No historical edge data is available for this position.</div>}
