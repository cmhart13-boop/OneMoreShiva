...
export default function HomeFantasyNews(){
  ...
  return <section className="og-fantasy-news" aria-label="ESPN fantasy football news">
    ...
    <div className="og-fantasy-news-grid">
      {articles.map((article,index)=><a key={`${article.url}-${index}`} className="og-fantasy-news-card" href={article.url} target="_blank" rel="noreferrer" aria-label={`Read on ESPN: ${article.headline}`}>
