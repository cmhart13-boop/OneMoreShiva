...
export default function GuideView() {
  ...
  return <>
    ...
    {view === 'strategy' && <>
      <div className="section-kicker">ROSTER CONSTRUCTION</div><h1>Draft Strategy</h1><p className="lede">Build for weekly ceiling without giving away value to the room.</p>
    ...
    {view === 'research' && <>
      ...
      {article ? <article className="article-detail"><button className="back-link" onClick={() => setArticleId(null)}>← Research notes</button><h2>{article.title}</h2><p>{article.body}</p>{article.players.length > 0 && <div className="tag-row player-tag-row">{article.players.map((name) => { const player = findPlayer(name); return <button type="button" key={name} onClick={() => openPlayer(player)}><PlayerAvatar playerId={player?.espnId || player?.id} name={name} /><span>{name}</span></button> })}</div>}</article> : <div className="article-grid">{researchArticles.map((item) => <button key={item.id} className="article-card" onClick={() => setArticleId(item.id)}><h2>{item.title}</h2><p>{item.body}</p><span>Open research →</span></button>)}</div>}
    </>}
    ...
    {view === 'luck' && <>
      ...
      <div className="strategy-grid"><button type="button" className="panel clickable-player" onClick={() => openPlayer(findPlayer('CeeDee Lamb'))}><div className="player-inline"><PlayerAvatar playerId={findPlayer('CeeDee Lamb')?.espnId || findPlayer('CeeDee Lamb')?.id} name="CeeDee Lamb" /><div className="player-inline-copy"><h2>CeeDee Lamb</h2><p>Rated among the strongest upward-regression profiles after an estimated ~2.7 PPG of negative-luck drag in 2025.</p></div></div></button><article className="panel"><h2>How to use it</h2><p>Luck never replaces talent, role or price. It is a tie-breaker when market cost and underlying opportunity already make sense.</p></article></div>
    </>}
