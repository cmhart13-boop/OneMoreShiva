...
new_fn=r"""def _home_nfl_news():
    ...
    try:
        ...
        for a in data.get("headlines") or []:
            ...
            img=next((x.get("url") for x in (a.get("images") or []) if isinstance(x,dict) and x.get("url")),None)
        ...
        for headline,web,img,description in articles:
            h=html.escape(headline);u=html.escape(web,quote=True);im=html.escape(img,quote=True);d=html.escape(description)
            desc=f'<div class="fantasy-news-desc">{d}</div>' if d else ''
            cards.append(f'<a class="fantasy-news-card" href="{u}" target="_blank" rel="noopener noreferrer"><div class="fantasy-news-img"><img src="{im}" alt=""></div><div class="fantasy-news-body"><div class="fantasy-news-headline">{h}</div>{desc}<div class="fantasy-news-meta">ESPN · Fantasy Football</div></div></a>')

        css=("<style>.fantasy-news-list{display:flex;flex-direction:column;gap:9px;margin:7px 0 14px}.fantasy-news-card{display:grid;grid-template-columns:108px minmax(0,1fr);overflow:hidden;text-decoration:none!important;color:#fff!important;background:#0e1821;border:1px solid #253644;border-radius:9px;min-height:84px}.fantasy-news-img{width:108px;min-height:84px;background:#172430;overflow:hidden}.fantasy-news-img img{display:block;width:100%;height:100%;object-fit:cover}.fantasy-news-body{padding:9px 10px 10px;min-width:0}.fantasy-news-headline{font-size:13px;font-weight:950;line-height:1.27;color:#fff}.fantasy-news-desc{font-size:10.5px;line-height:1.32;color:#9eacb7;margin-top:4px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.fantasy-news-meta{font-size:9px;color:#8fa0ae;margin-top:7px;font-weight:850;text-transform:uppercase;letter-spacing:.15px}@media(max-width:430px){.fantasy-news-card{grid-template-columns:96px minmax(0,1fr);min-height:80px}.fantasy-news-img{width:96px;min-height:80px}.fantasy-news-headline{font-size:12.5px}.fantasy-news-desc{font-size:10px}}</style><div class=\"fantasy-news-list\">"+"".join(cards)+"</div>")
