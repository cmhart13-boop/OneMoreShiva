...
.st-key-edge_position_filter [data-testid="stHorizontalBlock"]{gap:7px!important}.st-key-edge_position_filter .stButton>button{min-height:38px!important;padding:5px 8px!important;border-radius:11px!important;font-size:12px!important;font-weight:950!important}.st-key-edge_position_filter .stButton>button[kind="primary"]{border-color:rgba(240,216,143,.72)!important;background:linear-gradient(145deg,rgba(213,177,92,.24),rgba(213,177,92,.10))!important;color:#fff!important;box-shadow:0 0 18px rgba(213,177,92,.12)!important}.st-key-edge_position_filter .stButton>button[kind="secondary"]{background:#0d161d!important;border-color:#30404b!important;color:#9eabb3!important}
...
def _edge_panel(pool:pd.DataFrame,mode:str,position:str)->str:
    ...
    floor=mode=="floor"; stat="rate15" if floor else "boom25"; stat_label="15+ weeks" if floor else "25+ weeks"; position=position if position in {"QB","RB","WR","TE"} else "QB"
    ...
    return f'<div class="edge-panel"><div class="edge-panel-kicker">SHIVA SAYS</div><h3>{title}</h3><p class="edge-panel-copy">{copy}</p>{"".join(rows)}<div class="edge-method">Top 10 {position} · latest completed season only · ESPN Full PPR · minimum 8 games with verified weekly results. Rookies and players without enough historical games are not estimated.</div></div>'
...
def _set_edge_pos(pos:str)->None:
    st.session_state["shiva_edge_pos"]=pos


def _close_edge()->None:
    st.session_state.pop("shiva_edge_mode",None); st.session_state.pop("shiva_edge_pos",None)
...
def render_home_v2(players:pd.DataFrame,load_weekly,weekly_name_col,espn_ppr)->None:
    ...
    # Legacy deep links still open the correct panel, but normal filtering is session-state only.
    ...
    if query_pos in {"QB","RB","WR","TE"}:st.session_state["shiva_edge_pos"]=query_pos
    edge_mode=st.session_state.get("shiva_edge_mode"); edge_pos=st.session_state.get("shiva_edge_pos","QB")
    if edge_mode in {"floor","ceiling"}:
        with st.container(key="edge_position_filter"):
            cols=st.columns(4,gap="small")
            for i,pos in enumerate(("QB","RB","WR","TE")):
                with cols[i]:
                    st.button(pos,key=f"edge_pos_{edge_mode}_{pos}",type="primary" if edge_pos==pos else "secondary",use_container_width=True,on_click=_set_edge_pos,args=(pos,))
